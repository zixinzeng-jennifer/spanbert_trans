...
class DataProcessor(object):
    ...
    def _read_json(cls,input_file,quotechar=None):
        ...
        lines = []
        with open(input_file, "r", encoding="utf-8") as f:
            for text in json_lines.reader(f):
                lines.append(text)
...
class QAProcessor(DataProcessor):
    ...
    def get_train_examples(self, data_dir):
        """See base class."""
        return self._create_examples(
            self._read_json(os.path.join(data_dir, "new_train.json")), "train")

    def get_dev_examples(self, data_dir):
        """See base class."""
        return self._create_examples(
            self._read_json(os.path.join(data_dir, "new_dev.json")), "dev")

    def get_test_examples(self, data_dir):
        """See base class."""
        return self._create_examples(
            self._read_json(os.path.join(data_dir, "dev.jsonl")), "test")
...
def main(args):
    ...
    if args.do_train:
        logger.addHandler(logging.FileHandler(os.path.join(args.output_dir, "train.log"), 'w',encoding='utf-8'))
    else:
        logger.addHandler(logging.FileHandler(os.path.join(args.output_dir, "eval.log"), 'w',encoding='utf-8'))
    logger.info(args)
